 ...
 *
 */
public class FirstCrawlConfigurationAndSpecification {
	...
	public static void myCrawlRules(CrawljaxConfigurationBuilder builder) throws Exception {
		...
		if (browser.equals("PH")) {
			...
			throw new Exception("[ERROR]\tBrowserType not recognized (use CH, PH, FF)");
		}
		...
		File outFolder = new File(Settings.OUT_DIR1);
		if (outFolder.exists()) {
